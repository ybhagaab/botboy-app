...
const ROLE_TOOLS: Record<AgentRole, string[]> = {
  ...
  chat: ['get_today', 'list_projects', 'manage_area', 'manage_project', 'manage_page_layout', 'get_project_brain', 'get_channels', 'set_task_state', 'add_task', 'reject_evidence', 'discard_item', 'rebuild_brain', 'get_dashboard_sharing_status', 'list_analytics_dashboards', 'get_analytics_dashboard', 'create_analytics_dashboard', 'update_analytics_dashboard', 'configure_analytics_schedule', 'refresh_analytics_dashboard', 'mcp_status', 'mcp_profile_action', 'mcp_add_custom_server', 'mcp_update_custom_server', 'mcp_get_custom_server_config', 'mcp_call_tool', 'mcp_describe_tool', 'mcp_sql_list_presets', 'mcp_sql_get_schema_context', 'mcp_sql_list_schemas', 'mcp_sql_list_tables', 'mcp_sql_describe_table', 'mcp_sql_sample_data', 'mcp_sql_query', 'mcp_etl_job_run', 'mcp_etl_latest_run', 'mcp_etl_runs_for_job', 'mcp_etl_job', 'mcp_etl_profile_sql', 'mcp_etl_search', 'mcp_etl_diagnose_run', 'mcp_etl_download_results', 'mcp_etl_submit_run', 'mcp_etl_alter_run', 'mcp_etl_create_profile', 'mcp_etl_update_profile_sql', 'save_mcp_analysis', 'sharepoint_reply_comment', 'sharepoint_add_comment', 'sharepoint_update_document', 'sharepoint_edit_docx_body', 'sharepoint_create_document', 'list_documents', 'read_document', 'read_spreadsheet', 'list_nodes', 'get_node_items', 'search_items', 'send_chat_message', 'query_db', 'run_command', 'enrich_item', 'create_item', 'update_item', 'get_chat_messages', 'web_search', 'web_fetch', 'get_document_writing_guide', 'save_product_document', 'write_file', 'read_file', 'open_terminal', 'read_terminal', 'wait_for_terminal', 'send_terminal_input', 'close_terminal', 'refresh_toolchain'],
...
- ROUTING — SQL vs ETL (two different connections, fixed primacy): warehouse SQL, business analysis, dashboards, and any SELECT run through the SQL connection (mcp_sql_*) — ALWAYS. The mcp_etl_* tools are PRIMARY the moment the request is about DataCentral/Datanet/ETL: job runs, run status, schedules, profiles, "my ETL job", "the scheduled report", a datacentral.a2z.com URL, or submitting/restarting/creating ETL work. The ETL connection cannot run warehouse SQL by design — never try.
...
- ETL writes (mcp_etl_submit_run, mcp_etl_alter_run, mcp_etl_create_profile, mcp_etl_update_profile_sql) are real production pipeline changes: they run only on an explicit user request in this conversation (ownerRequested=true), one run at a time — diagnose before restarting (mcp_etl_diagnose_run). Batch/bulk pipeline operations and force-deps are structurally blocked; do not attempt or promise them.
