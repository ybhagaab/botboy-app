...
export function renderTodayView(context) {
  ...
  return `${pageHead(date, 'Good morning', 'Your explicit actions first, blocked work second, and meaningful evidence changes after that.', `<button class="button" type="button" data-action="open-command">${icon('search')} Find anything</button><button class="button primary" type="button" data-action="plan-day">${icon('sparkles')} Plan my day</button>`)}
    ...
    <div class="section-heading" id="today-attention"><div><h2>Needs your attention</h2><p>Pinned items first, then task state and decision/response language; substantive evidence only breaks ties.</p></div><span class="pill accent">${esc(countLabel(summary.attentionShown, summary.attentionCount, number, 'eligible'))}</span></div>
    ...
    <div class="section-heading" id="today-waiting"><div><h2>Waiting / blocked</h2><p>Recorded blocked tasks and blockers. BotBoy does not guess who owns the next move.</p></div><span class="pill warn">${esc(countLabel(summary.waitingShown, summary.waitingCount, number, 'recorded'))}</span></div>
    ...
    <div class="section-heading" id="today-changes"><div><h2>Meaningful changes</h2><p>${esc(changeWindow)}. Passive app activity, noise, incomplete captures, and lightweight browsing are excluded.</p></div><span class="pill blue">${esc(countLabel(summary.changesShown, summary.changeCount, number, 'changed projects'))}</span></div>
